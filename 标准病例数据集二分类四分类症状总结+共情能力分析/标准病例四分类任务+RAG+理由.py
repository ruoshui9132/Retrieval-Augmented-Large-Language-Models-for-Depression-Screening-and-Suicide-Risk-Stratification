...
def build_prompt(case_content: str, relevant_knowledge: List[Dict[str, str]]) -> str:
    ...
    3. 请提供详细的判断理由，说明你是如何根据DSM-5标准和对话内容得出上述结论的
...
1. **排除物质或躯体疾病诱因**：症状由酒精、药物（如镇静剂、降压药）滥用/戒断，或甲状腺功能减退、帕金森病、脑瘤、慢性疼痛等躯体疾病直接导致；
2. **排除其他精神障碍的“抑郁表现”**：例如，双相障碍患者的“抑郁发作”需先排除既往是否有“躁狂/轻躁狂发作”；精神分裂症患者的抑郁症状可能是精神病性症状的伴随表现，需优先诊断精神病性障碍；强迫症；创伤后应激障碍；广泛性焦虑障碍等
...
-3：近期自杀未遂
    ...
    抑郁症：无
    自杀风险：0
    判断理由：根据对话内容，患者表现出情绪低落、兴趣减退等核心症状，但症状数量未达到5项，且功能损害轻微，因此判断为无抑郁症。自杀风险为0是因为患者未表达任何自杀相关想法。
...
# 调用LLM API进行判断
def analyze_case(case_content: str, retriever: TaskOnlyKnowledgeRetriever, model_name: str = "Baichuan-M2", max_retries: int = 3) -> Tuple[str, int, str]:
    """调用LLM API分析单个案例，使用仅基于任务需求的RAG，返回抑郁症、自杀风险和判断理由"""
    # 检索相关知识
    ...
    for attempt in range(max_retries):
        try:
            ...
            reason_match = re.search(r'判断理由：(.+)', result, re.DOTALL)
            
            if depression_match and suicide_match and reason_match:
                return depression_match.group(1), suicide_match.group(1), reason_match.group(1).strip()
            else:
...
def process_cases(cases: Dict[int, str], retriever: TaskOnlyKnowledgeRetriever, model_name: str = "Baichuan-M2", delay_seconds: float = 1.0) -> List[Dict]:
    ...
    for i, (case_id, content) in enumerate(sorted(cases.items(), key=lambda x: x[0])):
        ...
        depression, suicide_risk, reason = analyze_case(content, retriever, model_name)
...
def save_results(results: List[Dict], output_file: str):
    """将判断结果保存到Excel文件，添加运行次数信息"""
    df = pd.DataFrame(results)
    # 确保列的顺序正确
    df = df[["案例编号", "抑郁症", "自杀风险", "判断理由"]]
    df.to_excel(output_file, index=False)
    print(f"结果已保存到 {output_file}")


def main():
    input_file = "标准病例对话154例.txt"       # 包含案例的文本文件路径
    ...
    for run_number in range(1, 2):  # 运行两次，run_number从1到2
        ...
        for model_name in models:
            ...
            output_file = f"标准病例{model_short_name}四分类任务_有RAG_第{run_number}次运行.xlsx"

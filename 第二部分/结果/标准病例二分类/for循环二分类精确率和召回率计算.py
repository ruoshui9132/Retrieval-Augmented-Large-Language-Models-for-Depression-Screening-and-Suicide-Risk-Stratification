...
def calculate_metrics(file_path):
    ...
    try:
        ...
        # 检查是否包含所需的列
        required_columns = ['案例编号', '有无抑郁症', '有无自杀风险', '抑郁症', '自杀风险']
        ...
        # 提取数据
        y_true_depression = df['抑郁症']
        y_pred_depression = df['有无抑郁症']
        y_true_suicide = df['自杀风险_text']
        y_pred_suicide = df['有无自杀风险']
        
        print("开始计算抑郁症二分类任务指标...")
        # 计算抑郁症四分类的指标
        report_depression = classification_report(y_true_depression, y_pred_depression, output_dict=True)
        
        print("开始计算自杀风险二分类任务指标...")
        # 计算自杀风险二分类的指标
...
def main():
    ...
    try:
        ...
        for filename in os.listdir(current_dir):
            if filename.endswith('.xlsx') and filename != '好大夫二分类指标计算结果.xlsx':
        ...
        # 将结果保存到Excel文件
        result_file = os.path.join(current_dir, '标准病例二分类精确率和召回率指标计算结果.xlsx')
